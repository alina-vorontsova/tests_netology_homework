...
def delete_doc(doc_number):
    doc_exist = check_document_existance(doc_number)
    doc_removed = False
    if doc_exist:
        for current_document in documents:
            doc_number = current_document['number']
...
def show_document_info(doc_number):
    for current_document in documents:
        if doc_number in current_document['number']:
            doc_type = current_document['type']
            doc_owner_name = current_document['name']
            doc_info = f'{doc_type} "{doc_number}" "{doc_owner_name}"'
            return doc_info
...
def secretary_program_start():
    ...
    l – (list) – команда, которая выведет информацию о документе в формате passport "2207 876234" "Василий Гупкин";
